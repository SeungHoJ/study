...
public class Ex06 {
	public static void main(String[] args) {
		...
		//배열의 i번째 글자 중에서, 짝수번째 글자가 알파벳 소문자 이면 대문자로 바꾸기
		for(int i = 0;i < arr.length;i++) {
			char ch = arr[i];
			if(i %2 == 0 && 'a'<= ch && ch <= 'z') {
				arr[i] = (char)(ch - 32);
		...
		for(int i = 0;i < arr1.length;i++) {	// 배열의 길이만큼 반복하면서
			...
			if('a'<= ch && ch <= 'z') {			// ch가 a와 z사이에 있다면(소문자이면
				arr1[i] = (char)(ch - 32);		// 배열의 값에 32를 빼서 대문자로 변경
			}
		}
		System.out.println(arr1);
		
		// 2) 주어진 문자열에서 숫자는 그대로 출력하고 숫자아닌 값은 출력하지 않고 줄 바꾸기
		String s3 = "9445-0230-1176-8878";
		char [] arr3 = s3.toCharArray();
		
		for(int i = 0;i < s3.length();i++) {
			char ch = arr3[i];
			if('0' <= ch && ch <= '9')	System.out.print(ch);
