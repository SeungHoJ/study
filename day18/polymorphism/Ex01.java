...
class Animal{		// 공통 기능과 속성은 슈퍼클래스에
	...
	// 서로 다른 동물의 공통점을 파악해보자
	// 다리가 4개, 꼬리가 있다, 털이 있다, 울음소리를 낸다
	void bark() {
		System.out.println(name + "...");
	}
}
class Dog extends Animal{  // 오버라이딩 및 개별 고유기능은 서브클래스에
	Dog(String name){
		super(name);
	}
	...
	void tailing() {		// 다른 Animal에서는 업는 Dog만의 고유기능(Dog가 아니면 동작하지 않는다)
		System.out.println(name+"이가 꼬리를 흔든다");
	}
}
class Cat extends Animal{  // 오버라이딩 및개별 고유 기능은 서브클래스에
	Cat(String name){
		super(name);
}	@Override
	void bark() {
...
public class Ex01 {
	public static void main(String[] args) {
		...
		// a1과 ob1은 서로 도일한 대상을 참조하고 있다
		// 다만, a1의 자료형은 Animal이고, ob1의 자료형은 Cat이다
		// a1으로 참조하면 Animal 클래ㅡ에 작서된 내용만 참조할 수 있고
		// ob1으로 참조하면 Cat클래스에 작성된 내용까지 모두 참조할 수 있다
		
//		a1.play();		// Animal 클래스에는 play()메서드가 없다
//		a2.tailing();	// Animal 클래스에는 tailing() 메서드가 없다
		
		a1.bark();		// 슈퍼클래스 Animal에는 name + ...으로 정의되어 있으나
		a2.bark();		// 각 서브클래스의 오버라이딩 내용으로 실행된다
		
		// upcasting : 서브클래스의 객체를
