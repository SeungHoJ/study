...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 정렬 및 기다 기능, 관리상의 불편함이 생길수 있다
		// 다양한 자료구조(Data Struct)를 이용하여 관리하게 되는데
		// 자바에서 기본 제공하는 형식이 있어서 이를 화룡ㅇ하면 편리하게 작업할 수 있다
		...
		// List를 implement하는 다른 클래스를 사용한다 (java api문서 참조)
		...
		// 리스트 및 컬렉션은 대부분 toString()이 오버라이딩되어있어ㅓ 바로 출력해도 내용을 확인할 수 있다
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
		
		// list.add(int index, E e) 함수는 지정한 인덱스에 값을추가한다
		list.add(1,30);
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
...
//		list.remove(30);						// 정수를 그냥 전달하면 index로 간주한다
		...
		// list.set(int index, E e) 함수는 지정한 위치의 값을 바꾸때 사용한다
		list.set(0, 60);	
		System.out.println(list);
		
		// 정렬함수가 내장되어 있거나, 다른 크래스를 이용하여 손쉽게 정렬할 수 있다
		list.sort(null);
		System.out.println(list);
