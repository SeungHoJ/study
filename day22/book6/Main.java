	...
	public static void main(String[] args) {
		...
		MAIN: while (true) {
			...
			case 3:
				...
				row = handler.updateBook(searchName);
				if(row == 0) {
				...
				case 1:
					...
					row = handler.updateBookFromName(searchName, changeName);
					System.out.println(row != 0 ? "수정 완료" : "수정 실패");
					break;
				case 2:
					System.out.println("변경할 작가 이름을 입력 : ");
					author = sc.nextLine();
					row = handler.updateBookFromAuthor(searchName,author);
					System.out.println(row != 0 ? "수정 완료" : "수정 실패");
					break;
				case 3:
					System.out.println("변경할 출판사 이름을 입력 : ");
					publisher = sc.nextLine();
					row = handler.updateBookFromPublisher(searchName, publisher);
					System.out.println(row != 0 ? "수정 완료" : "수정 실패");
					break;
				case 4:
					System.out.println("변경할 가격을 입력 : ");
					price = Integer.parseInt(sc.nextLine());
					row = handler.updateBookFromPrice(searchName, price);
					System.out.println(row != 0 ? "수정 완료" : "수정 실패");
					break;
				default:
